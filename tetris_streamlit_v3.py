"""
Streamlit 俄羅斯方塊 (手機優化版 v2)

針對手機瀏覽器（如 Safari）優化了按鈕佈局和大小。
v2: 縮小了遊戲區域並降低了刷新率，以解決手機上的捲動和閃爍問題。
...
# --- 遊戲核心常數 (v2 修改) ---
FIELD_WIDTH = 12
FIELD_HEIGHT = 16 # v2: 從 18 減少到 16，使其在手機上更短
BLOCK_SIZE = 20   # v2: 從 25 減少到 20，使其更窄
...
def initialize_game():
    ...
    if 'field' not in st.session_state:
        ...
        st.session_state.speed = 10 # v2: 從 20 調整為 10 (配合 0.1s 的 tick)
...
def handle_left():
    if not st.session_state.game_over and does_piece_fit(st.session_state.field, st.session_state.current_piece, st.session_state.current_rotation, st.session_state.current_x - 1, st.session_state.current_y):
...
def handle_down():
    if not st.session_state.game_over and does_piece_fit(st.session_state.field, st.session_state.current_piece, st.session_state.current_rotation, st.session_state.current_x, st.session_state.current_y + 1):
        st.session_state.current_y += 1
        st.session_state.speed_counter = 0 # 重置自動下降計時器

def handle_rotate():
    if not st.session_state.game_over and does_piece_fit(st.session_state.field, st.session_state.current_piece, st.session_state.current_rotation + 1, st.session_state.current_x, st.session_state.current_y):
...
def draw_board_svg():
    """使用 SVG 繪製遊戲板"""
    svg_parts = []
    # 使用 FIELD_WIDTH, FIELD_HEIGHT, BLOCK_SIZE
...
st.set_page_config(page_title="Streamlit 俄羅斯方塊", layout="centered", initial_sidebar_state="collapsed")
...
with r1_col2: # 中間欄
    st.button("⬆️ 旋轉", on_click=handle_rotate, use_container_width=True)

# 第二行：左、下、右
r2_col1, r2_col2, r2_col3 = st.columns([1, 1, 1])
with r2_col1:
    st.button("⬅️ 向左", on_click=handle_left, use_container_width=True)
with r2_col2:
    st.button("⬇️ 向下", on_click=handle_down, use_container_width=True)
with r2_col3:
    st.button("➡️ 向右", on_click=handle_right, use_container_width=True)
...
if force_down:
    ...
    if does_piece_fit(st.session_state.field, st.session_state.current_piece, st.session_state.current_rotation, st.session_state.current_x, st.session_state.current_y + 1):
        ...
        if st.session_state.piece_count % 10 == 0 and st.session_state.speed > 5: # v2: 速度最快 5 ticks (0.5s)
            st.session_state.speed -= 1

# --- 自動重新整理 ---
# 這是 Streamlit 遊戲循環的關鍵
time.sleep(0.1) # v2: 從 0.05 增加到 0.1，減少刷新率
st.rerun()